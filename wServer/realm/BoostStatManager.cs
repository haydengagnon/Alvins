...
namespace wServer.realm
{
    internal class BoostStatManager
    {
        ...
        private void ApplyEquipBonus(InventoryChangedEventArgs e)
        {
            ...
            for(var i = 4; i < _player.Inventory.Length; i++) {
                if (_player.Inventory[i] == null)
                    continue;
                foreach (var b in _player.Inventory[i].StatsBoostInInv)
                    IncrementBoost((StatsType)b.Key, b.Value);


                foreach (var b in _player.Inventory[i].StatsBoostPercInInv)
                    if (b.Value != 0)
                    {
                        var index = StatsManager.GetStatIndex((StatsType)b.Key);
                        IncrementBoost((StatsType)b.Key, (int)((_parent.Base[index] + _boost[index]) * (b.Value / 100.0)));
        ...
        private void ApplySacredEffects(InventoryChangedEventArgs e)
        {
            ...
            if (_player.CheckSacredEffect(SacredEffects.CollectorsEdition))
            {
                ...
                IncrementBoost(StatsType.Might, (int)(GetStat(StatsType.Might) * boost));
                ...
                IncrementBoost(StatsType.Protection, (int)(GetStat(StatsType.Protection) * boost));
            }
            if (_player.CheckSource())
            {
                IncrementBoost(StatsType.MaximumHP, (GetStat(StatsType.MaximumMP) + (GetStat(StatsType.Wisdom))));
            }
            ...
            {
                double boost0 = 0.02 * _player.Inventory.Where(inv => inv != null && inv.Legendary).Count();
                double boost1 = 0.04 * _player.Inventory.Where(inv => inv != null && inv.Godly).Count();
                double boost = boost1 + boost0;
                if (boost > 0)
                {
                    IncrementBoost(StatsType.MaximumHP, (int)(GetStat(StatsType.MaximumHP) * boost));
                    IncrementBoost(StatsType.MaximumMP, (int)(GetStat(StatsType.MaximumMP) * boost));
        ...
        private void ApplyMarkBoost()
        {
            ...
            switch (_player.Mark)
            {
                ...
                case 6:
                    ...
                    IncrementBoost(StatsType.Protection, (int)(GetStat(StatsType.Protection) * DEFAULT_STAT_INCREMENT));
                    ...
                    IncrementBoost(StatsType.Might, (int)(GetStat(StatsType.Might) * DEFAULT_STAT_INCREMENT));
                    IncrementBoost(StatsType.Luck, (int)(GetStat(StatsType.Luck) * DEFAULT_STAT_INCREMENT));
                    IncrementBoost(StatsType.Restoration, (int)(GetStat(StatsType.Restoration) * DEFAULT_STAT_INCREMENT));
                    break;
            }
        }
