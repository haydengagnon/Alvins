...
namespace wServer.realm.entities
{
    ...
    public partial class Player : Character, IContainer, IPlayer
    {
        ...
        public ItemStacker LGTokens { get; private set; }
        ...
        public Player(Client client, bool saveInventory = true) : base(client.Manager, client.Character.ObjectType)
        {
            ...
            LGTokens = new ItemStacker(this, 255, 0x0A24, 1, 50);
            Stacks = new ItemStacker[] { HealthPots, MagicPots, LGTokens };
        ...
        public override void Init(World owner)
        {
            ...
            var maxed = playerDesc.Stats.Where((t, i) => Stats.Base[i] >= t.MaxValue).Count() +
                        ...
                        || Stats.Base[i] >= t.MaxValue + 50).Count() : 0);
            //MarksActivate();
            if (spawnRegions.Any())
            {
                var rand = new Random();
            ...
            if ((owner.Name.Equals("MINNESOTA")
                || owner.Name.Equals("CAELID WILDS")
                || owner.Name.Equals("under the bed")
                || owner.Name.Equals("Necrop"))

                && maxed < 8)
            {
                SendError("You need to be 8/8 to access this zone!");
                ReconnectToNexus();
            }
            /*
            if ((owner.Name.Equals("Caelid Wilds")
                && maxed < 8))
            {
                SendError("You need the Tier 3 Unlocker to access this zone!");
                ReconnectToNexus();
            }
            if ((owner.Name.Equals("under the bed")
                && maxed < 11))
            {
                SendError("You need the Tier 3 Unlocker to access this zone!");
                ReconnectToNexus();
            ...
            */
            ...
            {
                int amount = (int)Math.Max(Math.Floor(Stats[10] / 90d * 6), 6);
        ...
        public void InInv(Item item)
        {
            if (Client.Player != null && item != null)
            {
                ...
                foreach (var pair in item.EffectInv)
        ...
        public void OnUnequipInv(Item item)
        {
            if (Client.Player != null && item != null)
            {
                foreach (var pair in item.EffectInv)
        ...
        public override void Tick() {
            ...
            if (!HasConditionEffect(ConditionEffects.Paused) && tickCount % 300 == 0) 
                FameCounter.Tick();
        ...
        public void CheckHitEffects()
        {
            ...
            if (CheckNil() && check < 50)
            {
                Stats.Boost.ActivateBoost[3].Push(500, true);
                Stats.ReCalculateValues();

                Owner.Timers.Add(new WorldTimer(5000, (w) =>
                {
                    if (w == null || w.Deleted) return;
                    Stats.Boost.ActivateBoost[0].Pop(500, true);
                    Stats.ReCalculateValues();
                }));
            }

            if (CheckVBow() && check < 50)
            {
                Stats.Boost.ActivateBoost[6].Push(1000, true);
                Stats.ReCalculateValues();

                Owner.Timers.Add(new WorldTimer(2500, (w) =>
                {
                    if (w == null || w.Deleted) return;
                    Stats.Boost.ActivateBoost[0].Pop(1000, true);
                    Stats.ReCalculateValues();
                }));
            }
            if (CheckRobe() && check < 1)
            {
                Stats.Boost.ActivateBoost[2].Push(50, true);
                Stats.ReCalculateValues();

                Owner.Timers.Add(new WorldTimer(4000, (w) =>
                {
                    if (w == null || w.Deleted) return;
                    Stats.Boost.ActivateBoost[2].Pop(50, true);
                    Stats.ReCalculateValues();
                }));
            }
            
            if (CheckLode() && check < 20)
            {
                Stats.Boost.ActivateBoost[2].Push(50, false);
                Stats.Boost.ActivateBoost[4].Push(10, false);
                Stats.Boost.ActivateBoost[5].Push(50, false);
                Stats.ReCalculateValues();

                Owner.Timers.Add(new WorldTimer(5000, (w) =>
                {
                    if (w == null || w.Deleted) return;
                    Stats.Boost.ActivateBoost[2].Pop(50, false);
                    Stats.Boost.ActivateBoost[4].Pop(10, false);
                    Stats.Boost.ActivateBoost[5].Pop(50, false);
                    Stats.ReCalculateValues();
                }));
            }
        }
        ...
        private int RestorationEffDuration()
        {
            return Stats[10] < 180 ? 0 : 1000 + (int)(1000 * Math.Min((Stats[10] - 180) / 20f, 9));
        }
        ...
        public void Damage(int dmg, Entity src, bool noDef)
        {
            ...
            if (RollSacredEffect(SacredEffects.AdrenalineRush, 20)) return;
        ...
        private bool NonPermaKillEnemy(Entity entity, string killer)
        {
            ...
            if (entity.Spawned)
                return false;
            foreach (var player in Owner.Players.Values)
                player.SendInfo("nonpermakillenemy");
            ReconnectToNexus();
            return true;
        }
        ...
        private bool Resurrection()
        {
            for (int i = 0; i < 20; i++)
            {
                ...
                if (chance == 1)
                {
                    Inventory[i] = null;
                    foreach (var player in Owner.Players.Values)
                        player.SendInfo($"{Name}'s {item.DisplayName} breaks and they disappear res proc");

                    ReconnectToNexus();
                    return true;
                }
                if (chance == 2 && item.Resurrects2)
                {
                    Inventory[i] = null;
                    foreach (var player in Owner.Players.Values)
                        player.SendInfo($"{Name}'s {item.DisplayName} breaks and they disappear res2 proc");

                    ReconnectToNexus();
                    return true;
                }
                if (chance == 3 && item.Resurrects3)
                {
                    Inventory[i] = null;
                    foreach (var player in Owner.Players.Values)
                        player.SendInfo($"{Name}'s {item.DisplayName} breaks and they disappear res3 proc");

                    ReconnectToNexus();
                    return true;
                }
                /*
                else if (chance == 2 && item.Resurrects2)
                {
                    Inventory[i] = null;
                    foreach (var player in Owner.Players.Values)
                ...
                else if (chance == 3 && item.Resurrects3)
                {
                    Inventory[i] = null;
                    foreach (var player in Owner.Players.Values)
                        player.SendInfo($"{Name}'s {item.DisplayName} breaks and he disappears");
        ...
        {
            if (Owner.Name.Equals("Admins Arena") || Owner.Name.Equals("Balancer Station"))
            {
                Random rnd = new Random();
                ...
                switch (num)
                {
                    case 1:
                        foreach (var player in Owner.Players.Values)
                            player.SendInfo($"GOD SAVE THE RAVE! {Name} GTFO");
                        break;
                }
                foreach (var player in Owner.Players.Values)
                ReconnectToNexus();
        ...
        private void AnnounceDeath(string killer)
        {
            ...
            var maxed = playerDesc.Stats.Where((t, i) => Stats.Base[i] >= t.MaxValue).Count() +
                (AscensionEnabled ?
                    playerDesc.Stats.Where((t, i) => Stats.Base[i] >= t.MaxValue + 10 ||
                       Stats.Base[i] >= t.MaxValue + 50).Count() : 0); //ghetto code

            var deathMessage = "<" + Name + "> died to " + killer +
                " (" + maxed + (AscensionEnabled ? "/24 " : "/8 ") + playerDesc.ObjectId +
                ", " + Fame + " Base Fame)";

            // notable deaths
            if ((maxed >= 8 || Fame >= 0) && !Client.Account.Admin)
            {
