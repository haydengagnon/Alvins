    ...
    public class Entity : IProjectileOwner, ICollidable<Entity>
    {
        ...
        public virtual void Tick()
        {
            ...
            if (CurrentState != null && Owner != null)
                if (!HasConditionEffect(ConditionEffects.Stasis) && !TickStateManually && (Owner.Players.Count != 0 || ConditionEffects != 0))
                    TickState();
        ...
        public void ValidateAndMove(float x, float y)
        {
            ...
            var pos = new Vector2();
            ResolveNewLocation(x, y, ref pos);
            Move(pos.X, pos.Y);
        }

        private void ResolveNewLocation(float x, float y, ref Vector2 pos)
        {
            ...
            while (!done)
            {
                ...
                CalcNewLocation(pos.X + dx * ds, pos.Y + dy * ds, ref pos);
                tds = tds + ds;
            }
        }

        private void CalcNewLocation(float x, float y, ref Vector2 pos)
        {
            float fx = 0;
            float fy = 0;
