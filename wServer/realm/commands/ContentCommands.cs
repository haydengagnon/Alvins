...
namespace wServer.realm.commands
{
    ...
    {
        public BalanceCommand() : base("balance", Perms.Mod, alias: "nerftime")
        {
        }
        ...
        private bool SpawnJson(Player player, string json)
        {
            ...
            if (props.spawns != null)
                foreach (var spawn in props.spawns)
                {
                    ...
                    var count = 1;
                    if (spawn.count > count && player.Rank < 80)
                        if (spawn.count <= 200)
    ...
    {
        public GimmeCommand() : base("gimme", Perms.Mod,alias: "give")
        {
        }
        ...
        protected override bool Process(Player player, string args)
        {
            ...
            if (availableSlot != -1)
            {
               ...
               // Give2Discord("Give Log", $"Give Log: [{player.Name}] has given themselves a [{item.DisplayName}]");
                return true;
            }
    ...
    {
        public AscendCommand() : base("ascend", Perms.HeadMod)
        {
        }
        ...
        protected override bool Process(Player player, string args)
        {
            ...
            {
                player.SendError("You must consume a Lost Scripture #2 to use this command!");
                return false;
            }
                ...
                player.Stats.Base[9] = pd.Stats[9].MaxValue + 10;
                player.Stats.Base[10] = pd.Stats[10].MaxValue + 10;
                player.Stats.Base[11] = pd.Stats[11].MaxValue + 10;
