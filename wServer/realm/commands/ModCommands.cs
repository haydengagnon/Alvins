...
namespace wServer.realm.commands
{
        ...
        protected override bool Process(Player player, string args)
        {
            foreach (var i in player.Manager.Clients.Keys)
            {
                if (i.Account.Name.EqualsIgnoreCase(args))
                {
                    ...
                    //LogToDiscord("Kick Log", $"Kick Log: [{player.Name}] has kicked [{i.Account.Name}]");
                    i.Disconnect();
    ...
    internal class MuteCommand : Command
    {
        ...
        protected override bool Process(Player player, string args)
        {
            ...
            //LogToDiscord("Mute Log", $"Mute Log: [{player.Name}] has muted [{name}] for {timeout} minutes");
    ...
    internal class UnMuteCommand : Command
    {
        ...
        protected override bool Process(Player player, string name)
        {
            ...
            //LogToDiscord("Unmute Log", $"Unmute Log: [{player.Name}] has unmuted [{name}]");
    ...
    {
        public BanAccountCommand() : base("ban", Perms.Owner)
        {
        }
        ...
        protected override bool Process(Player player, string args)
        {
           ...
           // LogToDiscord("Ban Log", $"Ban Log: [{player.Name}] has banned [{acc.Name}] for {bInfo.banReasons}");
    ...
    {
        public BanIPCommand() : base("banip", Perms.HeadMod, alias: "ipban")
        {
        }
        ...
        protected override bool Process(Player player, string args)
        {
            ...
            //LogToDiscord("Ban IP Log", $"Ban IP Log: [{player.Name}] has ip banned [{acc.Name}] for {reason}");
    ...
    {
        public UnBanAccountCommand() : base("unban", Perms.HeadMod)
        {
        }
        ...
        protected override bool Process(Player player, string args)
        {
            ...
            if (banned && ipBanned)
            {
                //LogToDiscord("Unban Log", $"Unban Log: [{player.Name}] has unbanned [{acc.Name}]");
    ...
    {
        public HideCommand() : base("hide", Perms.HeadMod, alias: "h")
        {
        }
    ...
    {
        public WhereAllCommand() : base("whereall")
        {
        }
    ...
    internal class FetchBanCommand : Command
    {
        public FetchBanCommand() : base("fetchban", Perms.Mod, alias: "getban")
        {
        }
