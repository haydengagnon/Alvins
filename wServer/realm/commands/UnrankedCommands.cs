...
namespace wServer.realm.commands
{
    ...
    //internal class InsigCommand : Command
    //{
    //    public InsigCommand() : base("insig")
    //    {
    //    }

    //    protected override bool Process(Player player, string args)
    //    {
    //        if (!player.Owner.Name.Equals("Tavern"))
    //        {
    //            player.SendError("You are not in the tavern.");
    //            return false;
    //        }

    //        player.SendInfo($"{player.Owner.ChallengeCount}/5");
    //        return true;
    //    }
    //}

    internal class LootCheck : Command
    {
        public LootCheck() : base("lootboost", alias: "lb") { }
        ...
        protected override bool Process(Player player, string args)
        {
            ...
            if (player.Rank >= 10 && player.Rank <= 19)
            {
                donorBoost = 10;
            }
            if (player.Rank >= 20 && player.Rank <= 29)
            {
                donorBoost = 20;
            }
            if (player.Rank >= 30)
            ...
            var serverBoost = player.Manager.Config.serverSettings.lootMultiplier * 100;
            var fortuneMod = player.Manager.Config.serverSettings.fortuneMultiplier / 100;
            var fortuneBoost = (player.Stats.Boost[14] / 1) * fortuneMod;
            var lootDropBoost = player.LDBoostTime > 0 ? 50 : 1 - 1;
            var totalBoost = serverBoost + fortuneBoost + lootDropBoost + donorBoost + guildBoost;
            if (serverBoost > 100)
            {
                player.SendInfo($"An event is currently giving an extra {serverBoost}%!");
            }
            if (player.Rank >= 10)
            {
                player.SendInfo($"Your donor rank is currently giving an extra {donorBoost}%!");
            }
            if (fortuneMod > 1)
            {
                player.SendInfo($"An event is currently giving an extra {fortuneMod}x fortune modifier!");
            }
            if (player.GuildRank >= 0)
            {
                player.SendInfo($"Your guild is currently giving an extra {guildBoost}%!");
            }
            player.SendInfo($"{fortuneBoost}% from Fortune stat. You have a total: {totalBoost}% loot boost!");
            return true;
        ...
        protected override bool Process(Player player, string args)
        {
            ...
            if (world != null)
            {
                if (world.Invites.Contains(player.Name.ToLower()))
                {
                    ...
                    if (world is BalancerStation && player.Rank < 40 || world.Name is "KrakenLair" || world.Name is "TheHollows" || world.Name is "HiddenTempleBoss" || world.Name is "FrozenIsland" || world.Name is "Admins Arena" || world.Name is "Castle" || world.Name is "Oryx's Chamber" || world.Name is "Wine Cellar" || world.Name is "AldraginesHideout" || world.Name is "UltraAldraginesHideout" || world.Name is "Sincryer's Gate" || world.Name is "Ultra Sincryer's Gate" || world.Name is "Nontridus" || world.Name is "NontridusUltra" || world.Name is "Core of the Hideout" || world.Name is "Ultra Core of the Hideout" || world.Name is "Keeping of Aldragine" || world.Name is "KeepingUltra" || world.Name is "Zol Secret Shop" || world.Name is "BastilleofDrannol" || world.Name is "Ultra Bastille of Drannol" || world.Name is "Hunter Cave" || world.Name is "Ultra Hunter Cave" || world.Name is "Twisted Trials" || world.Name is "Ultra Twisted Trials" || world.Name is "Twisted Trials Cont" || world.Name is "Ultra Twisted Trials Cont" || world.Name is "Twisted Domain" || world.Name is "Ultra Twisted Domain" || world.Name is "The Steps" || world.Name is "Ultra The Steps" || world.Name is "The Steps 2" || world.Name is "Ultra The Steps 2" || world.Name is "SummoningPoint" || world.Name is "UltraSummoningPoint" || world.Name is "Drannol Secret Shop" || world.Name is "Ultra Drannol Secret Shop" || world.Name is "Chamber of Malgor" || world.Name is "Dream World" || world.Name is "The Void")
                    {
