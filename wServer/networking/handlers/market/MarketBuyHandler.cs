    ...
    class MarketBuyHandler : PacketHandlerBase<MarketBuy>
    {
        ...
        protected override void HandlePacket(Client client, MarketBuy packet)
        {
            client.Manager.Core.addPendingAction(() =>
            {
                ...
                int val = changedSellerAmount % 10000;
                ...
                if (seller != null)
                {
                    seller.Player.SendInfo($"{player.Name} has just bought your {item.ObjectId} for {data.Price} gold! Check your gift chest for the gold!");
