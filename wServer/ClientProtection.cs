...
namespace wServer
{
	public class ClientProtection
	{
		...
		public ClientProtection(bool enabled, string[] tokens)
		{
			IsEnabled = enabled;
			Tokens = tokens;
			if (IsEnabled) GenerateHashs();
		}

		private void GenerateHashs()
		{
			if (!File.Exists("Alvin.swf"))
			{
				IsEnabled = false;
				return;
			}

			Hashs = (string[])Tokens.Clone();

			byte[] swf = File.ReadAllBytes("Alvin.swf");
