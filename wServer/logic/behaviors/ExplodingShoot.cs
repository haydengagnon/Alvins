...
namespace wServer.logic.behaviors
{
    internal class ExplodingShoot : CycleBehavior
    {
        ...
        {
            int cool = (int?)state - (int)CoreConstant.worldLogicTickMs ?? -1; // <-- crashes server due to state being null... patched now but should be looked at.
            ...
            if (cool <= 0)
            {
                ...
                if (host.HasConditionEffect(ConditionEffects.Dazed)) count = (int)Math.Ceiling(_count / 2.0);
